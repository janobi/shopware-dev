...
When set to `0` (`SHOPWARE_ES_USE_LANGUAGE_ANALYZER=0`), search queries use `sw_whitespace_analyzer` instead, providing more accurate and precise search results with fewer matches.

**Example:**
```bash
# For more precise search (fewer, more accurate results)
SHOPWARE_ES_USE_LANGUAGE_ANALYZER=0

# For broader search (default behavior - more fuzzy results)
SHOPWARE_ES_USE_LANGUAGE_ANALYZER=1
```
